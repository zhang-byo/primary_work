...
def read_excel():
    ...
    with open("telecom_train.csv", "r") as csvfile:
        ...
        chrun_prom_0, chrun_prom_1 = count_box_width_variable(reader, 6)
...
def count_box_width_variable(reader, location):
    ...
    for item in reader:
        if item[1] != 'churn':
            data_peakMinDiff.append(float(item[location]))
            data_churn.append(int(float(item[1])))
    data_peakMinDiff = pd.cut(data_peakMinDiff, k, labels=range(k))
    for i in range(len(data_churn)):
        if data_churn[i] == 0 and data_peakMinDiff[i] == 0:
            chrun_variable_0[0] += 1
        if data_churn[i] == 0 and data_peakMinDiff[i] == 1:
            chrun_variable_0[1] += 1
        if data_churn[i] == 0 and data_peakMinDiff[i] == 2:
            chrun_variable_0[2] += 1
        if data_churn[i] == 0 and data_peakMinDiff[i] == 3:
            chrun_variable_0[3] += 1
        if data_churn[i] == 1 and data_peakMinDiff[i] == 0:
            chrun_variable_1[0] += 1
        if data_churn[i] == 1 and data_peakMinDiff[i] == 1:
            chrun_variable_1[1] += 1
        if data_churn[i] == 1 and data_peakMinDiff[i] == 2:
            chrun_variable_1[2] += 1
        if data_churn[i] == 1 and data_peakMinDiff[i] == 3:
            chrun_variable_1[3] += 1
    return chrun_variable_0, chrun_variable_1

def count_log_variable(reader, location, length):
    chrun_variable_0 = [0 for i in range(length)]
    chrun_variable_1 = [0 for i in range(length)]
    data_churn = []
    data_peakMinDiff = []
    for item in reader:
        if item[1] != 'churn':
            value = np.math.log(float(item[location]))
            if item[1] == '0.0' and value > 1.6555712946839494 and value < 3.332204510175204:
                chrun_variable_0[0] += 1
            if item[1] == '0.0' and value >= 3.332204510175204 and value < 4.948372529916523:
                chrun_variable_0[1] += 1
            if item[1] == '1.0' and value > 1.6555712946839494 and value < 3.332204510175204:
                chrun_variable_1[0] += 1
            if item[1] == '1.0' and value >= 3.332204510175204 and value < 4.948372529916523:
                chrun_variable_1[1] += 1
    return chrun_variable_0, chrun_variable_1
